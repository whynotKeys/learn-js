...
// 카카오api 같이 ts용 파일 제공 없는 api 쓰려면 직접 전역 타입 선언 필요
// 전역(global)로 사용하는 타입 선언 : module에서만 가능
declare global {
  interface Window { // Window에 kakao(any) 속성을 추가
    kakao : any;
  }
}
...
function find(){
  navigator.geolocation.getCurrentPosition((currentPosition) =>{ // 성공 시 실행할 콜백함수
    console.log(currentPosition);
      const position : Position = {
        lat : currentPosition.coords.latitude, // 위도
        lng : currentPosition.coords.longitude, // 경도
        accuracy : currentPosition.coords.accuracy, // 위경도 오차 (m단위
      };
...
const options = { //지도를 생성할 때 필요한 기본 옵션
    center: new window.kakao.maps.LatLng(33.450701, 126.570667), //지도의 중심좌표. // kakao 앞에 window 붙임
